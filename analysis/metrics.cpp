...
std::string gerarCabecalhoCSV() {
    return "Estrutura,Documentos,NumeroPalavrasTotais,NumeroPalavrasDistintas,TempoTotalInsercao,TempoMedioInsercao,TempoTotalBusca,TempoMedioBusca,Altura,MenorGalho,MaiorGalho,ComparacoesInsercao,ComparacoesBusca,NumeroRotacao\n";
}

std::string gerarLinhaCSV(const Metrics& m) {
    return m.estrutura + "," +
           std::to_string(m.qtdDocumentos) + "," +
           std::to_string(m.qtdPalavrasTotais) + "," +
           std::to_string(m.qtdPalavrasDist) + "," +
